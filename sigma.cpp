...
class Sigma {
	...
	Sigma(double thetaC, double phiP, double thetaP, double _nF) : mThetaC(thetaC), nF(_nF) {
		...
		double cosThetaC = TMath::Cos(mThetaC);
		double tanThetaC = TMath::Tan(mThetaC);
		double sinThetaP = TMath::Sin(mThetaC);
  }

	private :
		double trkBeta = 1./(TMath::Cos(mThetaC)*nF);

		if(trkBeta() > 1) trkBeta() = 1;                 //protection against bad measured mThetaCer  
		if(trkBeta() < 0) trkBeta() = 0.0001;            //

		const double getTrkBeta() { return trkBeta;}
		...
		//   Returns: absolute error on Cerenkov angle, [radians]    
			...
			double alpha =cost-tanThetaP*cosfd*sint;                                                 // formula (11)
			double k = 1.-nF*nF+alpha*alpha/(getTrkBeta()*getTrkBeta());        // formula (after 8 in the text)
			if (k<0) return 1e10;
			double mu =sint*sinf+tanThetaP*(cost*cosfd*sinf+sinfd*cosf);                             // formula (10)
			double e  =sint*cosf+tanThetaP*(cost*cosfd*cosf-sinfd*sinf);                             // formula (9)
		...
		//   Returns: absolute error on Cerenkov angle, [radians]    
			...
			double alpha =cost-tanThetaP*cosfd*sint;                                                 // formula (11)
			double dtdn = cost*nF*getTrkBeta()*getTrkBeta()/(alpha*tanThetaP);                    // formula (12)
		...
		//   Returns: absolute error on Cerenkov angle, [radians]    
			...
			double c1 = 1./(1.+ eTr*k/(alpha*alpha*cosThetaP*cosThetaP));                              // formula (13.a)
			double c2 = getTrkBeta()*TMath::Power(k,1.5)*tanThetaP*lambda/(gapThick*alpha*alpha);  // formula (13.b)
			double c3 = (1.+eTr*k*getTrkBeta()*getTrkBeta())/((1+eTr)*alpha*alpha);                                // formula (13.c)
			double c4 = TMath::Sqrt(k)*tanThetaP*(1-lambda)/(gapThick*getTrkBeta());               // formula (13.d)
